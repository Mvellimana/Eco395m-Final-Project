...
for i in range(587):
    for j in range(50):
        response = requests.get(str(df["business_info"][i+136]["url"]) + "&start="+ str(j) + str(0))
        ...
        for ii in range(len(reviews)):
            x = {
                "restaurant_name" : df["business_info"][i+136]["name"],
                "number" : ii+1+(j*10),
    ...
    with open(os.path.join("artifacts", "yelp_all_reviews_" + df["business_info"][i+136]["name"] + ".csv"), 'w', encoding = "utf-8", newline='') as output_file:
